...
<body class="bg-gray-50 flex">
    ...
    <div class="flex-1 min-h-screen">
        <div class="max-w-7xl mx-auto p-6">
            ...
            <div class="rounded-lg shadow-sm overflow-hidden">
                ...
                for ($i = 5; $i < $totalRows; $i +=5) {
                    ...
                    <h3 class="text-lg font-medium text-gray-700">Data Pinjaman</h3>
            </div>

            <!-- List Table -->
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="pinjamTable">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">No</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Barang</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah Pinjam</th>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Di Konfirmasi Oleh</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <form action="{{ route('pinjam.reject', $pinjam->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-md transition duration-200">Reject</button>
                                </form>
                                @else
                                <span class="text-gray-500">{{ ucfirst($pinjam->status) }}</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
        ...
        <div class="w-full flex justify-beetwenmt-4">
            {{ $pinjams->appends([request('rows')])->links('vendor.pagination.custom') }}
        </div>
    </div>
    </div>
    </div>
